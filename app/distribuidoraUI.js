...
export default function DistribuidoraUI() {
  ...
  const comprar = async () => {
    try {
      //verificar que el cliente haya completado sus datos
      ...
      // ✅ Armar el mensaje de WhatsApp con TODO el carrito
      let whatsappMessage = `🛒 Nuevo pedido:\n\n👤 Cliente: ${nombreCliente}\n🏠 Dirección: ${direccionCliente}\n\n`;

      carrito.forEach(item => {
        whatsappMessage += `- ${item.name} x${item.cantidad} = $${item.price * item.cantidad}\n`;
      });
      whatsappMessage += `\n💰 Total: $${totalCarrito}`;

      // Enviar mensaje por WhatsApp (solo una vez)
      const whatsappUrl = `https://api.whatsapp.com/send?phone=5491164369179&text=${encodeURIComponent(whatsappMessage)}`;
      ...
      <main className="container mx-auto p-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {productos.map(producto => (
            ...
            >
              <img
                src={producto.imageUrl ? producto.imageUrl : 'https://via.placeholder.com/150'}
                alt={producto.name}
