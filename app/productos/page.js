...
export default function Productos() {
  ...
  const agregarProducto = async (e) => {
    ...
    await fetch(`${API_URL}/productos`, {
      ...
      body: JSON.stringify({
        ...
        stock: Number(stock),
        price: Number(price),
        imageUrl: imageUrl || "", // si no hay imagen, string vacío
      }),
    });
  ...
  const guardarEdicion = async (e) => {
    ...
    await fetch(`${API_URL}/productos/${productoEditando.id}`, {
      method: "PUT",
      body: formData,
    });
        ...
        <ul className="grid grid-cols-1 gap-6 sm:grid-cols-2">
          {productosFiltrados.map((p) => (
            ...
            >
              {p.imageUrl && (
                <img
                  src={p.imageUrl ? p.imageUrl : 'https://via.placeholder.com/150'}
                  alt={p.name}
